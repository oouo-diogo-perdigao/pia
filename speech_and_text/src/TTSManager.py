...
def tts_worker_process(task_queue, result_queue):
    ...
    while True:
        try:
            ...
            if time.monotonic() - last_used >= IDLE_TIMEOUT_QWEN:
                if qwen_model is not None:
                    logging.info(
                        "[WORKER] Timeout de 5 minutos atingido. Descarregando Qwen3-TTS da memória..."
                    )
                    if qwen_model:
                        del qwen_model
                        qwen_model = None
                        logging.info("[WORKER] Modelo Qwen3-TTS descarregado.")
            if time.monotonic() - last_used >= IDLE_TIMEOUT_KOKORO:
                if kokoro is not None:
                    logging.info(
                        "[WORKER] Timeout de 10 minutos atingido. Descarregando Kokoro da memória..."
                    )
                    if kokoro:
                        del kokoro
                        kokoro = None
                        logging.info("[WORKER] Modelo Kokoro descarregado.")
            if not kokoro and not qwen_model:
                break
            continue
        ...
        if cmd == "GENERATE":
            ...
            try:
                ...
                if voice in TTSManager.qwen_voices():
                    logging.info(
