...
DEFAULT_SPEED = float(os.getenv("DEFAULT_SPEED", "0.95").strip())
    ...
    def do_POST(self):
        try:
            ...
            if self.path == "/speak":
                ...
                style = payload.get("style", None)  # <--- Captura a instrução de estilo
                ...
                PLAYER.add_job(
                    text, voice, speed, device=device, style=style
                )  # <--- Envia style
