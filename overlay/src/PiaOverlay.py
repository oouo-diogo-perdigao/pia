...
from .config import SAT_SERVER_URL
...
def quiet_qt_messages(mode, context, message):
    # Ignora a mensagem de erro de DPI do Windows
    if "SetProcessDpiAwarenessContext" in message:
        return
    # Mantém o comportamento padrão para os demais logs
...
class PiaOverlay(QWidget):
    ...
    def mousePressEvent(self, event):
        if event.button() == Qt.MouseButton.LeftButton:
            ...
            # ADIÇÃO: Notifica o servidor SAT para parar imediatamente
            try:
                import requests
                from .config import SAT_SERVER_URL

                requests.post(f"{SAT_SERVER_URL}/stop", timeout=1)
            except Exception:
                pass
...
class PiaStateMachine:
    ...
    def trigger_blink(self):
        # Piscar os olhos (detectado silêncio / fim do áudio para enviar ao SAT)
        overlay = get_overlay()
        if overlay:
            overlay.blink()
...
def run_overlay_app():
    ...
    # Instale o manipulador antes de inicializar o QApplication
    qInstallMessageHandler(quiet_qt_messages)

    # Caminho para os arquivos na raiz ou na pasta overlay
    # base_dir = Path(__file__).resolve().parent.parent.parent
    # svg_file_path = base_dir / "pia.svg"
    # html_file_path = Path(__file__).resolve().parent / "overlay.html" # ou ajuste o caminho conforme onde salvar o html

    # svg_raw = svg_file_path.read_text(encoding="utf-8") if svg_file_path.exists() else "<h3>Arquivo pia.svg não encontrado</h3>"

    # if html_file_path.exists():
    #     html_template = html_file_path.read_text(encoding="utf-8")
    #     # Injeta os valores dinâmicos no template HTML
    #     svg_content = html_template.format(
    #         window_size=WINDOW_SIZE,
    #         sat_server_url=SAT_SERVER_URL,
    #         svg_raw=svg_raw
    #     )
    # else:
    #     svg_content = "<html><body><h3>Arquivo overlay.html não encontrado</h3></body></html>"
...
<script>
    ...
    function startStreaming() {{
        ...
        eventSource = new EventSource("{SAT_SERVER_URL}/status/stream");
...
"""
    ...
    # Configura o tratamento para o Ctrl+C (SIGINT) fechar o app suavemente sem traceback
    import signal

    def handle_sigint(signum, frame):
        # Fecha a aplicação Qt de forma limpa ao receber Ctrl+C
        app.quit()
